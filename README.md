...
**Si reconstruís el índice desde la demo pública:** el filesystem de Vercel es de solo lectura fuera de `/tmp`, así que el índice recién construido **no se guarda en disco** — queda activo en memoria (`retriever.set_store()`) solo para la instancia serverless que atendió ese pedido. Tu próxima pregunta puede o no caer en esa misma instancia (Vercel no garantiza afinidad de sesión). No es un bug, es routing serverless — por eso el índice pre-construido committeado sigue siendo el fallback confiable para cualquier otra instancia.
